...
| **Internal-synthesis workflows — backend/headless** (`viral_epitope_evidence_review`, `synthesize_query`) | apecx LLM backend: local Ollama (`localhost:11434` default) **or** `APECX_LLM_BASE_URL` — **no remote default** | ❌ (needs the backend LLM configured) | ✅ available (Ollama up this run) | `test_viral_epitope_evidence_review.py`: evidence-only, **5-section output contract**, structural-no-hit-named, streaming==headless, design-without-approval, design HITL loop — **12 passed, 1 skip** (see below) |
...
The *backend* internal-synthesis path (the bundled `viral_epitope_evidence_review`
