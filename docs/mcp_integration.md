...
### Local database tool (`database_statistics`)

`database_statistics` requires domain dataset files on the local filesystem (the `query_*` /
`resolve_entity` DB tools were retired 2026-06-15 — Globus-first; `harmonized_search` supersedes
them). Run `apecx-setup` once to provision the data, then set:
...
### Entity resolution (synonym dictionary)

The synonym dictionary powers fast canonical-IRI / taxon matching for `harmonized_search` and the
workflows' entity resolution (e.g. a bare virus name → taxon ID feeding `viral_epitope_analysis`).
Without it, resolution falls back to slower substring matching.
...
For per-tool **input/output shapes**, query the LIVE schema — `describe_workflow(name)` and
`await build_server().list_tools()` return the current, authoritative signatures. (The previous
hand-written ~430-line per-tool catalog was removed 2026-06-27: it documented the retired
...
`analyze_*` — and had drifted out of date. Two hand-maintained tool lists is exactly how that drift
happened; `docs/architecture.md` §4 is now the single curated overview.)
