...
| G38-WA-1 | G38 (HTTP backend adapter for ToolExecutionStep) | `SynonymCacheLookupStep` + `VerifiedSynonymWritebackStep` post directly to control-plane HTTP endpoints; bypass `ToolExecutionStep`'s adapter dispatch. | `composition/steps/synonym_cache.py` | **HTTPBackendAdapter shipped 2026-05-10 (G38)**; FULL closure requires replacing both steps with `ToolExecutionStep` + UTDs + post-processing for result partitioning. Transport-only swap was attempted 2026-05-11 and reverted — the existing probe-batch tests inject `client.post()`-only mocks that don't satisfy the adapter's `client.request()` shape. Closure remains a structural refactor, not a transport swap. | These steps cannot ride the G15 UnifiedToolDescriptor / G28 capability-token surface; HTTP is hardcoded |
