...
**The "Active workarounds: 0" claim was wrong before this 2026-05-09 update; the truthful count was 4. G33/G35/G39/G44 retired 2026-05-09; G38-WA-1 retired 2026-05-11 (adapter refinement); G40-WA-1 retired 2026-05-11 (delegation to `nanobrain.library.runtime.workspace_root`). **Current truthful count: 2 active** (G34-WA-1, G36-WA-1) — both Tier 4 backlog, both documentation-only / transitional patterns.**
...
| G38-WA-1 | G38 (HTTP backend adapter for ToolExecutionStep) | apecx-mcp `286a72e` + nanobrain `bb54312` | 2026-05-11 | Both `SynonymCacheLookupStep` and `VerifiedSynonymWritebackStep` now dispatch through `HTTPBackendAdapter` (G38). Adapter refined to use httpx's per-method helpers (`.post`/`.get`/`.put`/`.patch`/`.delete`) so existing probe-batch mock-injection tests stay compatible — mocks were extended with `status_code` / `headers` / `text` to honor the full httpx response surface. The step classes survive (vs. dissolving into `ToolExecutionStep + PartitionStep`) because the partition / 409-handling business logic is domain-specific and doesn't generalize; the workaround was "direct httpx in step body", and the new code path routes through the adapter (uniform error handling, `X-Nanobrain-Run-Namespace` header propagation, mock-friendly per-method dispatch). A future structural refactor to ToolExecutionStep is possible but no longer required for closure. |
| G40-WA-1 | G40 (workspace-root helper) | apecx-mcp commits + nanobrain `67f3f1b` | 2026-05-11 (interim) / 2026-05-16 (full retirement) | **INITIAL (2026-05-11):** `_workspace.py.resolve_workspace_root` delegated to `nanobrain.library.runtime.workspace_root.locate_workflow_root` (G40) for the marker-walk + env-var override; only the apecx-specific marker (`apecx-mcp-integration`) and the `fallback_depth` last-resort were preserved at the wrapper. **FULLY RETIRED (2026-05-16):** nanobrain G40 helper grew `fallback_depth=N` + file-path-`start=` support (nanobrain commit `67f3f1b`), so the entire apecx wrapper was deleted; the 2 callers now call `locate_workflow_root(start=__file__, markers=["apecx-mcp-integration"], env_var="APECX_WORKSPACE_ROOT", fallback_depth=5)` directly. The 6 apecx tests for the wrapper were retired; equivalent coverage now lives in `nanobrain/tests/unit/test_g40_workspace_root.py` (16 tests). |
