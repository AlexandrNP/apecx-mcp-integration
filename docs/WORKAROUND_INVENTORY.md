...
| G40-WA-1 | G40 (workspace-root helper) | (this session apecx-mcp commit) | 2026-05-11 | `_workspace.py.resolve_workspace_root` now delegates to `nanobrain.library.runtime.workspace_root.locate_workflow_root` (G40) for the marker-walk + env-var override; only the apecx-specific marker (`apecx-mcp-integration`) and the `fallback_depth` last-resort behavior are preserved at the wrapper. Caller signature unchanged. Note: the prior compound-marker check (require apecx-mcp-integration AND a sibling among data/nanobrain/_workspace_notes) is relaxed to the single-marker check since the compound check was defensive coding without a known false-positive case; the framework helper's marker model is intentionally simple. |
