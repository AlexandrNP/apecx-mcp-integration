...
def test_probe_1272_composer_config_yml_temperature_zero_default():
    ...
    assert temp == 0.0, (
        f"composer temperature is {temp} — non-zero defeats determinism / reproducibility"
    )
...
def test_probe_1273_workflow_yamls_use_consistent_class_path_format():
    ...
    # violin_bvbrc retired 2026-06-15; validate the surviving workflow.
    yaml_dir = (
    ...
    for ypath in yaml_dir.rglob("*.yml"):
        ...
        if cls and isinstance(cls, str):
            assert "/" not in cls and "\\" not in cls, (
                f"{ypath.name}: class path looks like a file path: {cls!r}"
            )
...
def test_probe_1276_composer_prompts_dir_listed_in_module():
    ...
    composer_py = REPO_ROOT / "src" / "apecx_integration" / "composition" / "composer.py"
...
def test_probe_1277_workflow_link_yaml_keys_have_no_typos():
    ...
    for link_id, link_def in raw.get("links", {}).items():
        ...
        assert not unknown, f"link {link_id!r} has unknown config keys: {unknown}"
        ...
        for marker in suspicious:
            if marker in text:
                ...
                if marker == "FIXME" and "`FIXME`" in text:
                    continue
                pytest.fail(f"{prompt_file.name}: unresolved marker {marker!r}")
