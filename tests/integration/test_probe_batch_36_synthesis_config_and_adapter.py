...
def test_probe_958_max_publications_zero_accepted():
    ...
    cfg = SynthesisConfig.model_validate({"system_prompt": "x", "max_publications": 0})
...
def test_probe_959_min_distinct_citations_zero_accepted():
    ...
    cfg = SynthesisConfig.model_validate({"system_prompt": "x", "min_distinct_citations": 0})
...
def test_probe_960_empty_system_prompt_either_rejected_or_accepted_explicitly():
    ...
    try:
        cfg = SynthesisConfig.model_validate({"system_prompt": ""})
...
def test_probe_962_load_default_config_surfaces_yaml_parse_error(tmp_path, monkeypatch):
    ...
    # yaml.YAMLError is the documented parse-failure surface; pin
    # it rather than ``Exception`` so a future silent-fallback to
    # defaults doesn't accidentally satisfy the assertion.
...
def test_probe_970_e2e_keyword_filter_excludes_short_keywords():
    ...
    assert out == [], (
        f"short-keyword filter failed: matched {len(out)} rows on length-only-junk keywords"
    )
...
def test_probe_973_publication_doi_with_internal_brackets_rejected_in_render():
    ...
    2026-05-11: the synthesizer now FAILS-FAST in strict mode on
    bracketed/whitespace DOIs — the silent-failure shape this probe
    was guarding against is closed at the renderer boundary, not at
    the regex matcher. The contract (matched against the rejection
    message):

      'doi {x!r} contains characters (']', '[', or whitespace) that
       break the citation extraction pattern; the LLM cannot produce
       a valid inline citation for this DOI.'

    Probe converted from "return-tuple shape mismatch" to
    "FAIL-FAST in strict mode" — that's what the probe title
    (``_rejected_in_render``) was always intending."""
    ...
    with _pytest.raises(ValueError, match=r"contract violation"):
        _render_publications([pub], cap=1, strict=True)

    # Lenient mode MUST NOT raise (operator opt-out exists for
    # dirty-pipeline scenarios). Verify the skip-with-warning path.
    rendered, allowed = _render_publications([pub], cap=1, strict=False)
    # The bracketed DOI is skipped — no token added to ``allowed``.
    assert pub["doi"] not in " ".join(allowed), (
        "lenient mode must SKIP bracketed DOIs, not pass them "
        "through. The grounding gate downstream would fire on a "
        "pass-through."
    )
...
def test_probe_977_e2e_pipeline_test_module_is_importable_under_pytest():
    ...
    mod = importlib.import_module("tests.integration.test_e2e_rag_pipeline_against_ollama")
...
def test_probe_978_synthesizer_module_import_does_not_eagerly_load_yaml():
    ...
    mod = importlib.import_module("apecx_integration.agents.rag_synthesis.synthesizer")
